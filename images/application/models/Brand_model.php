...
class brand_model extends CI_Model {
    ...
    function get_autocomplete($condition = array(),$select='id,brand_user_id') {
        $this->db->select('id as value,brand_user_id as name');
    ...
    function get_all_ajax_search_brand($row, $limit, $condition = array(), $order_by_fieled, $order_by_type = "asc") {
        //$this->db->where('blog_status','E');
        if ($condition) {
            $this->db->like($condition);
            //$this->db->where($condition);
...
//                       Edit=2;		
        $this->db->from($this->table);
        $condition = array('brand_url' => $gallery_url);
        if ($condition) {
            $this->db->where($condition);
        }
        $query = $this->db->get();
