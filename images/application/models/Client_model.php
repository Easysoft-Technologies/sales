		...
		{   
			...
			if ($query->num_rows() > 0){
			  ...
			  return array();
                 ...
                 function get_all_ajax_search_client($row,$limit,$condition=array(),$order_by_fieled,$order_by_type="asc")
		{   
			//$this->db->where('blog_status','E');
			if($condition)
			{
...
//                       Edit=2;		
			$this->db->from($this->table);
                        $condition = array('client_url' => $gallery_url);
			if($condition){
					$this->db->where($condition);
				}
			$query = $this->db->get();
    ...
    function get_autocomplete($condition = array(),$select='id,brand_user_id') {
        $this->db->select('id as value,client_user_id as name');
        $this->db->from($this->table);
        $this->db->like($condition);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
