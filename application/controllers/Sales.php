...
class sales extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->max_size = 3000; //3MB
       // $this->max_size = 18000; //2MB
    ...
    function reports_all($row = 0) {
        $limit = '20';
        $config['base_url'] = site_url('sales/ajax_search_report_sales');
        $config['full_tag_open'] = '<ul class="pagination custom-pagntion">';
        $config['full_tag_close'] = '</ul>';
    ...
    function not_upload_rows($sales_url = '') {
       ...
       $data['client_data'] = $this->obj_sales->get_all($condition);

        
         $this->load->view('sales/not_upload_rows', $data);
    }
    ...
    function report_client($row = 0) {
       ...
       $this->session->unset_userdata('search_report_type');
        ...
        $this->session->unset_userdata('sort_checkbox');
        $limit = '20';
        $config['base_url'] = site_url('sales/ajax_search_report_sales');
        $config['full_tag_open'] = '<ul class="pagination custom-pagntion">';
        $config['full_tag_close'] = '</ul>';
    ...
    function ajax_search_report_sales($row = 0) {
...
//        $family_id = $this->session->userdata('search_family');
        ...
        if ($from_date && $to_date) {
         ...
         if ($this->input->post('sort_checkbox')) {
            $data['sort_checkbox'] = $this->input->post('sort_checkbox');
            $sess_data['sort_checkbox']=$this->input->post('sort_checkbox');
        } else {
            $data['sort_checkbox'] = array();
        }
        if ($brand) {
    ...
    function ajax_pagination($row = 0) {
        ...
        $from_date = $this->session->userdata('date_from');
        $to_date = $this->session->userdata('date_to');
        $order_id = $this->session->userdata('order_id');
        
        $sort_checkbox = $this->session->userdata('sort_checkbox');
        if ($sort_checkbox) {
            $data['sort_checkbox'] = $sort_checkbox;
        } else {
            $data['sort_checkbox'] = array();
        }
        // $from_date = strtotime($from_date . '00:00:00');
                ...
                foreach ($csv as $row) {
                    if ($i == 0) {
                        ...
                        if ($row[0] && $row[1] && $row[3] && $row[2] && $row[6] && $row[8]) {
                            ...
                            //order                            
                            $condition_order_product = array('order_id' => $order_id, 'product_id' => $product_id,'quantity'=>$row[5]);
                            $order_item_id = $this->obj_common->countrows('order_items', $condition_order_product);
                            if ($order_item_id == 0) {
                                $total = $row[9] * $row[5];
                                $total = abs(filter_var($row[10], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION));
...
//print_r($cells);
    ...
    function sales_report() {
        ...
        $start_month = $this->input->get_post('from_month');
        $end_month = $this->input->get_post('to_month');
        $year = $this->input->get_post('year');
        $from_date1 = strtotime('1-' . $start_month . '-' . $year);
        $to_date1 = strtotime('1-' . $end_month . '-' . $year);

        $from_date = date('Y-m-d', strtotime('first day of this month', $from_date1));
        $to_date = date('Y-m-d', strtotime('last day of this month', $to_date1));
        $data['start_month'] = $start_month;
        $data['end_month'] = $end_month;
        $data['year'] = $year;
        $data['sales_representative_id'] = $user;
        $data['client_id'] = $client;
        $data['brand_id'] = $brand;
        $search_report_type = $this->input->get_post('search_report_type');
         if ($search_report_type == 1) {
           $this->load->view('sales/sales_report_representative', $data);
        }
        if ($search_report_type == 2) {
            //$msg = $this->load->view('sales/sales_report_brand', $data, true);
           $this->load->view('sales/sales_report_brand', $data);
        }       
        if ($search_report_type == 3) {
           $this->load->view('sales/sales_report_client', $data);
        }
    }
...
//        3 Client
        $status = 0;
        $search_report_name = $this->input->get_post('search_report_name');
        $search_report_type = $this->input->get_post('search_report_type');
        $sort_checkbox=array();
        if ($this->input->get_post('sort_checkbox')) {
            $data['sort_checkbox'] = $this->input->get_post('sort_checkbox');
           $sort_checkbox = $this->input->get_post('sort_checkbox');
        } else {
            $data['sort_checkbox'] = array();
        }
        ...
        if ($this->input->get_post('download_pdf')) {
            //redirect('sales/download_pdf');

            $message = $this->message($condition,$data);
            $status = 1;
            //echo $message;
            $this->generate_pdf($message, 'Report');
        }

        if ($this->input->get_post('print_pdf')) {
        ...
        if ($this->input->get_post('excel')) {

            $row = 0;
            $limit = '';
            $query = $this->obj_sales->get_all_data_csv($row, '', $condition, 'orders.id', 'asc',$sort_checkbox);
            $filename = time() . '.csv';
            $this->load->dbutil();
            $this->load->helper('file');
            $this->load->helper('download');
    ...
    function message($condition = array(),$data) {
        $row = 0;
        $limit = '';
        $data['row'] = 0;
